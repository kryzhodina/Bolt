...
DROP TABLE if exists bolt.d_date;

CREATE TABLE bolt.d_date
(
...
ALTER TABLE bolt.d_date ADD CONSTRAINT d_date_date_dim_id_pk PRIMARY KEY (date_dim_id);

CREATE INDEX d_date_date_actual_idx
  ON bolt.d_date(date_actual);

COMMIT;

INSERT INTO bolt.d_date
SELECT trim(TO_CHAR(datum,'yyyymmdd'))::INT AS date_dim_id,
       ...
       trim(TO_CHAR(datum,'W'))::INT AS week_of_month,
       EXTRACT(week FROM datum) AS week_of_year,
       trim(TO_CHAR(datum,'YYYY"-W"IW-')) || EXTRACT(isodow FROM datum) AS week_of_year_iso,
       ...
       trim(TO_CHAR(datum,'mmyyyy')) AS mmyyyy,
       trim(TO_CHAR(datum,'mmddyyyy')) AS mmddyyyy,
       CASE
